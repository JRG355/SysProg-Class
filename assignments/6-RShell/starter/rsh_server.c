...
int exec_client_requests(int cli_socket)
{
    ...
    while (1) {
        while (1) {
            io_size = recv(cli_socket, p, RDSH_COMM_BUFF_SZ, 0);
            if (io_size == -1) {
                return ERR_RDSH_COMMUNICATION;
            }
        ...
        bi_cmd = exec_built_in_cmd(cmd_list.commands);
...
int send_message_eof(int cli_socket)
{
    ...
    rc = send(cli_socket, &RDSH_EOF_CHAR, sizeof(char), 0);
    if (rc != sizeof(char)) {
        return ERR_RDSH_COMMUNICATION;
    }
...
int send_message_string(int cli_socket, char *buff)
{
    ...
    rc = send(cli_socket, buff, cmd_len, 0);
    if (rc < cmd_len) {
        return ERR_RDSH_COMMUNICATION;
    }
...
int rsh_execute_pipeline(int cli_sock, command_list_t *clist)
{
    ...
    for (int i = 0; i < clist->num; i++) {
        // TODO this is basically the same as the piped fork/exec assignment, except for where you
        // connect the begin and end of the pipeline (hint: cli_sock)
